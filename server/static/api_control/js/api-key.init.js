let checkAll = document.getElementById("checkAll"),
    ...
    for (let e = document.querySelectorAll('.form-check-all input[type="checkbox"]'),
             t = document.querySelectorAll('.form-check-all input[type="checkbox"]:checked').length,
             n = 0; n < e.length; n++)
        e[n].checked = this.checked,
        e[n].checked ? e[n].closest("tr").classList.add("table-active") :
        e[n].closest("tr").classList.remove("table-active");

    document.getElementById("remove-actions").style.display = 0 < t ? "none": "block"}), 8),

options = {
    valueNames: ["id", "name", "createBy", "apikey", "status", "create_date", "expiry_date"],
    page: perPage,
    pagination: !0,
...
xhttp.onload = function() {
    ...
    Array.from(e).forEach(function(e) {
        apiKeyList.add({
            ...
            name: e.name,
            createBy: e.createBy,
            apikey: '<input type="text" class="form-control apikey-value" readonly value="' + e.apikey + '">',
            status: isStatus(e.status),
            create_date: e.create_date,
            expiry_date: e.expiry_date
        }),
        apiKeyList.sort("id", {order: "desc"}),
        "Active" === e.status ? document.querySelector(".disable-btn").innerHTML = "Disable": "Disable" === e.status && (document.querySelector(".disable-btn").innerHTML = "Enable"), refreshCallbacks()
    }), apiKeyList.remove("id", '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ001</a>')
},

xhttp.open("GET", ""),
xhttp.send(),
document.getElementById("api-key-modal").addEventListener("show.bs.modal",
function(e) {
    e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Rename API name",
        ...
        document.getElementById("edit-btn").style.display = "block") : e.relatedTarget.classList.contains("create-btn") ?
        (document.getElementById("exampleModalLabel").innerHTML = "Create API Key",
            ...
            document.getElementById("add-btn").style.display = "none") : e.relatedTarget.classList.contains("regenerate-api-btn") &&
...
let idField = document.getElementById("apikeyId"),
...
function generateApiID() {
    ...
    return "sadalksjklasdjljksaasasddljasd"
...
document.getElementById("api-key-modal").addEventListener("hidden.bs.modal",
function() {
    clearFields()
});

document.querySelector("#apiKeyList").addEventListener("click",
function() {
    ischeckboxcheck()
});
...
function ischeckboxcheck() {
    Array.from(document.getElementsByName("chk_child")).forEach(function(n) {
        ...
        function(e) {
            ...
            let t = document.querySelectorAll('[name="chk_child"]:checked').length;
...
function refreshCallbacks() {
    Array.from(editBtns).forEach(function(e) {
        ...
        function(e) {
            ...
            Array.from(e).forEach(function(e) {
                let t = (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML;
    ...
    Array.from(removeBtns).forEach(function(e) {
        ...
        function(e) {
            ...
            Array.from(e).forEach(function(e) {
                let t = (deleteid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild;
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click",
                function() {
...
function deleteMultiple() {
    ids_array = [];
    let e, t = document.querySelectorAll(".form-check [value=option1]");
...
document.getElementById("createApi-btn").addEventListener("click",
function(e) {
    let t;
    if (0 === apiKeyNameField.value.length) return (t = document.getElementById("api-key-error-msg")).classList.remove("d-none"),
    !(t.innerHTML = "Please enter api key name");
    0 < apiKeyNameField.value.length &&
    (document.getElementById("apikey-element").style.display = "block", e.target.style.display = "none", document.getElementById("add-btn").style.display = "block")
}),

addBtn.addEventListener("click", function(e) {
    let t = document.getElementById("api-key-error-msg");
    if (t.classList.remove("d-none"), 0 === apiKeyNameField.value.length)
        return ! (t.innerHTML = "Please enter api key name");
    "" !== apiKeyNameField.value && "" !== apiKeyField.value && (
        apiKeyList.add({
...
function(e) {
    ...
    let t = apiKeyList.get({
        id: idField.value
    });
    Array.from(t).forEach(function(e) { (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && e.values({
