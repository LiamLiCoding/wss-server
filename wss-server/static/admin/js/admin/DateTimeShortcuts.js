...
'use strict';
{
    const DateTimeShortcuts = {
        ...
        // Return the current time while accounting for the wss-server timezone.
        now: function() {
            const serverOffset = document.body.dataset.adminUtcOffset;
            if (serverOffset) {
        ...
        addTimezoneWarning: function(inp) {
            ...
            if (timezoneOffset > 0) {
                message = ngettext(
                    'Note: You are %s hour ahead of wss-server time.',
                    'Note: You are %s hours ahead of wss-server time.',
                    timezoneOffset
                );
            }
            else {
                timezoneOffset *= -1;
                message = ngettext(
                    'Note: You are %s hour behind wss-server time.',
                    'Note: You are %s hours behind wss-server time.',
                    timezoneOffset
                );
            }
